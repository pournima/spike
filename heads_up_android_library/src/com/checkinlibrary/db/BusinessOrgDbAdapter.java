...
public class BusinessOrgDbAdapter extends DbAdapter {
	...
	ContentValues createContentValues(Integer businessId, Integer orgId,
			String name) {
	...
	public long create(Integer businessId, Integer orgId, String name) {
		ContentValues initialValues = createContentValues(businessId, orgId,
				name);
		return super.create(initialValues);
	}

	public boolean update(long rowId, Integer businessId, Integer orgId,
			String name) {
		ContentValues updateValues = createContentValues(businessId, orgId,
				name);
		return super.update(rowId, updateValues);
	}

	public List<BizOrgSupportsResult> getList(Integer businessId) {
		BusinessOrgDbAdapter bus_org_db = new BusinessOrgDbAdapter(
				this.context, this.dbName);
		Cursor busOrgC = bus_org_db.fetchAll(
				"business_id=" + businessId.toString(), null);
		...
		if (busOrgC.moveToFirst()) {
			do {
				BizOrgSupportsResult org = new BizOrgSupportsResult(
						busOrgC.getInt(idColumn),
						busOrgC.getString(orgNameColumn));
				orgs.add(org);
