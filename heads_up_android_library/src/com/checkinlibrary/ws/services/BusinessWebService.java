...
public class BusinessWebService implements WebServiceIface {
	public static List<BusinessResult> getNearby(Context context,
		...
		if (response != null)
			Log.e("CHECKINFORGOOD", "Business Result for lat "
					+ location_params[0] + " lon " + location_params[1] + ": "
					+ response.toString());
	...
	public static List<BusinessResult> search(CharSequence searchText,
			Location location) {
		Double[] latLon;
		AppStatus appStatus = new AppStatus();
		...
		if (location == null) {
			...
			latLon = new Double[] { location.getLatitude(),
					location.getLongitude() };
		}

		List<BusinessResult> res = null;
		WebService webService = new WebService(BASE_URL
				+ "/mobile_user/search_bus.json");
		...
		if (response == null) {
			...
			Log.e("CHECKINFORGOOD",
					"Business search response: " + response.toString());
		}
		...
		try {
			...
			res = new Gson().fromJson(response,
					(java.lang.reflect.Type) typeOfListOfBusinessResult);
		} catch (Exception e) {
