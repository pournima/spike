...
public class TwitterShare extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		bIsFromSocialLogin=getIntent().getBooleanExtra(PrepareRequestTokenActivity.IS_FROM_WHERE, false);
	...
	public class MyTimer extends CountDownTimer {
        ...
        public void onFinish() {
        	...
        	if(!bIsActivityFinished)
        		Toast.makeText(getApplicationContext(),"Twitter is not responding, try after some time..",Toast.LENGTH_SHORT).show();
        	bIsActivityFinished=true;
        	finish();
        }
	...
	public void gotoTwitterDialog(){
		if(!bIsActivityFinished){
			...
			i.putExtra(PrepareRequestTokenActivity.IS_FROM_WHERE, bIsFromSocialLogin);
			startActivity(i);
			bIsActivityFinished=true;
			finish();
		}else{
	...
	public class AuthenticateTask extends AsyncTask<String, Void, Boolean> {
	    ...
	    public AuthenticateTask(TwitterShare context,String token,String secret) {
	        this.token = token;
	    	this.secret = secret;
	    	mContext=context;
	    }
